...
router.get('/', obtenerUsuario, async function (req, res, next) {
    try {
        ...
        const [rows] = await connection.query('select * from reporteDesaparicion where id_usuario=?', [idUser]);
        if (rows.length === 0) {
...
router.patch('/',obtenerUsuario, async function (req, res, next) { 
    ...
    const [update] = await connection.query('UPDATE reporteDesaparicion SET descripcion = ?  WHERE id_reporte= ?', [description, idReport]);
